...
class MultiHeadAttention(nn.Module):
    def __init__(self, heads, d_model, dropout=0.1):
        ...
        assert d_model % self.h == 0, "Embedding dimension must be divisible by number of heads."
...
class Encoder(nn.Module):
    ...
    def __init__(self, vocab_size, d_model, N, heads, dropout, max_seq_length):
        super().__init__()
        self.N = N
        self.embed = Embedder(vocab_size, d_model)
        self.pe = PositionalEncoder(
            d_model, dropout=dropout, max_seq_length=max_seq_length)
...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab, trg_vocab, d_model, N, heads, dropout, max_seq_length):
        super().__init__()
        self.encoder = Encoder(src_vocab, d_model, N,
                               heads, dropout, max_seq_length)
        self.decoder = Decoder(trg_vocab, d_model, N,
                               heads, dropout, max_seq_length)
...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        with torch.no_grad():
            ...
            mask = torch.nonzero(
                target.data == self.padding_idx, as_tuple=False)
            if mask.dim() > 0:
...
def bleu(valid_src_data, valid_trg_data, model, SRC, TRG, device, k, max_strlen):
    ...
    for sentence in valid_src_data:
        pred_trg = translate_sentence(
            sentence, model, SRC, TRG, device, k, max_strlen)
...
train_src_data, train_trg_data = read_data(
    opt['train_src_data'], opt['train_trg_data'])
valid_src_data, valid_trg_data = read_data(
    opt['valid_src_data'], opt['valid_trg_data'])

SRC, TRG = create_fields(opt['src_lang'], opt['trg_lang'])
train_iter = create_dataset(train_src_data, train_trg_data,
                            opt['max_strlen'], opt['batchsize'], opt['device'], SRC, TRG, istrain=True)
valid_iter = create_dataset(valid_src_data, valid_trg_data,
                            opt['max_strlen'], opt['batchsize'], opt['device'], SRC, TRG, istrain=False)
src_pad = SRC.vocab.stoi['<pad>']
trg_pad = TRG.vocab.stoi['<pad>']
# text = "Encoder của mô hình transformer có thể bao gồm nhiều encoder layer tượng tự nhau."

# # Khởi tạo Embedder
# # số lượng từ vựng trong bộ từ vựng. Trong trường hợp này, có tối đa 1000 từ khác nhau.
# vocab_size = 1000
# # kích thước của vector biểu diễn cho mỗi từ trong không gian vector. Trong trường hợp này, kích thước vector biểu diễn cho mỗi từ là 300.
# d_model = 512
# embedder = Embedder(vocab_size, d_model)

# # Tạo đầu vào
# tokens = nltk.word_tokenize(text)
# word2idx = {word: i+1 for i, word in enumerate(set(tokens))}
# input_tensor = torch.LongTensor([word2idx[word] for word in tokens])

# # Lấy embedding của đầu vào
# embedding = embedder(input_tensor)

# # In ra embedding
# # print(embedding)

# pos_encoder = PositionalEncoder(d_model=d_model, max_seq_length=16)
# pos_encoded = pos_encoder(embedding)

# # In ra PositionalEncoder
# # print(pos_encoded)

# # In ra MultiHeadAttention
# multiHead = MultiHeadAttention(heads=8, d_model=d_model)
# # print(multiHead)

# # In ra EncoderLayer
# encoderLayer = EncoderLayer(d_model=d_model, heads=8)
# # print(encoderLayer)

# # In ra DecoderLayer
# decoderLayer = DecoderLayer(d_model=d_model, heads=8)
# # print(decoderLayer)

# encoder = Encoder(vocab_size=vocab_size, d_model=d_model,
#                   N=6, heads=8, dropout=0.1, max_seq_length=16)

# decoder = Decoder(vocab_size=vocab_size, d_model=d_model,
#                   N=6, heads=8, dropout=0.1, max_seq_length=16)

# # transformer = Transformer(src_vocab=vocab_size,
...
model = Transformer(len(SRC.vocab), len(
    TRG.vocab), opt['d_model'], opt['n_layers'], opt['heads'], opt['dropout'],max_seq_length=16)
...
criterion = LabelSmoothingLoss(
    len(TRG.vocab), padding_idx=trg_pad, smoothing=0.1)
...
for epoch in range(opt['epochs']):
    ...
    bleuscore = bleu(valid_src_data[:500], valid_trg_data[:500],
                     model, SRC, TRG, opt['device'], opt['k'], opt['max_strlen'])
    print('epoch: {:03d} - iter: {:05d} - valid loss: {:.4f} - bleu score: {:.4f} - time: {:.4f}'.format(
        epoch, i, valid_loss, bleuscore, time.time()))
